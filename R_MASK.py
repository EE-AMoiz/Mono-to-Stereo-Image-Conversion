...
class TwoPassRightView:
    """
    Two-pass forward-warp that creates RIGHT-view style disocclusions (holes on right of objects),
    with explicit translation control.

    Pass 1: shift RIGHT by 1.0 * disp/scale_factor
    Pass 2: shift LEFT  by translation * disp_tmp/scale_factor
    ...
    def __init__(self, img_bgr, disp, scale_factor=6.0, translation=2.0, mask_dilate=0):
        ...
        self.right_valid = None
        self.mask = None
        self.masked_right_view = None

    def run(self):
        # ---- Pass 1: RIGHT shift (reference, fixed at 1.0) ----
        ...
        # ---- Pass 2: LEFT shift (this is your controllable translation) ----
        self.right_view, self.right_valid, _ = forward_warp_x(
            src_img=self.tmp_img,
            src_disp=self.tmp_disp,
            scale_factor=self.scale_factor,
            direction_sign=-1,
            shift_mult=self.translation,
            valid_src=self.tmp_valid,     # CRITICAL FIX
            write_disp=False
        )

        # Holes come ONLY from landing map (not pixel color)
        self.mask = (self.right_valid == 0).astype(np.uint8) * 255
        ...
        self.masked_right_view = self.right_view.copy()
        self.masked_right_view[self.mask == 255] = 0
    ...
    def write_all(self, out_root, frame_idx):
        ...
        cv2.imwrite(os.path.join(out_root, "masked_image", f"{frame_idx:05d}.png"), self.masked_right_view)
...
def process_images(orig_folder, disp_folder, save_path, workers=None, start_index=0,
    ...
    print(f"NOTE: net shift ~ (translation-1) * disp/scale_factor (leftward if translation>1).")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser("Two-pass warp RIGHT-view synthesis with translation control.")
    parser.add_argument("--orig", required=True, help="Folder with original (left) images")
    ...
    parser.add_argument("--translation", type=float, default=2.0,
                        help="Controls final left shift strength (right view). Use >1.0 for net left shift.")
