    ...
    with torch.no_grad():
        with scope("Sampling"):
            for image, mask in tqdm(zip(images, masks)):
                ...
                predicted_image_pil = Image.fromarray(predicted_image.astype(np.uint8))
                predicted_image_pil = predicted_image_pil.resize(original_size, Image.Resampling.LANCZOS)
                predicted_image_pil.save(outpath)